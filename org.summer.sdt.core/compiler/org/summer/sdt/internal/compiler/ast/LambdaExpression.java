...
import org.summer.sdt.internal.compiler.flow.ExceptionInferenceFlowContext;
...
//		}
		...
		for (int i = 0; i < len; i++) {
			...
			if (ourTagBits == 0L) {
				...
				if (ourTagBits == TagBits.AnnotationNonNull) { // requested @NonNull not provided
					...
					currentScope.problemReporter().illegalRedefinitionToNonNullParameter(this.arguments[i], this.descriptor, inheritedAnnotationName);
				}
			}			
		}
	}
	...
	public boolean isPotentiallyCompatibleWith(TypeBinding targetType, Scope skope) {
		...
		/* 15.12.2.1: A lambda expression (搂15.27) is potentially compatible with a functional interface type (搂9.8) if all of the following are true:
		       鈥�The arity of the target type's function type is the same as the arity of the lambda expression.
		       鈥�If the target type's function type has a void return, then the lambda body is either a statement expression (搂14.8) or a void-compatible block (搂15.27.2).
		       鈥�If the target type's function type has a (non-void) return type, then the lambda body is either an expression or a value-compatible block (搂15.27.2).
		*/
	...
	public ReferenceBinding getTypeBinding() {
		...
		class LambdaTypeBinding extends ReferenceBinding {
			...
			public char[] computeUniqueKey() {
				return LambdaExpression.this.descriptor.computeUniqueKey();
			}
			public String toString() {
				StringBuffer output = new StringBuffer("()->{} implements "); //$NON-NLS-1$
				output.append(LambdaExpression.this.descriptor.sourceName());
				output.append('.');
