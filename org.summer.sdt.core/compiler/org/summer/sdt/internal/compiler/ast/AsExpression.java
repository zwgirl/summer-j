...
import static org.summer.sdt.internal.compiler.ast.ExpressionContext.CASTING_CONTEXT;
...
public class AsExpression extends Expression {
	 ...
	 */
	public static void checkNeedForAsAs(BlockScope scope, AsExpression enclosingCast) {
	...
	/**
	 * Complain if as expression is cast, but not actually needed, int i = (int)(Integer) 12;
	 * Note that this (int) cast is however needed:   Integer i = 0;  char c = (char)((int) i);
	 */
	public static void checkNeedForAsCast(BlockScope scope, AsExpression enclosingCast) {
		if (scope.compilerOptions().getSeverity(CompilerOptions.UnnecessaryTypeCheck) == ProblemSeverities.Ignore) return;
	
		AsExpression nestedCast = (AsExpression) enclosingCast.expression;
		if ((nestedCast.bits & ASTNode.UnnecessaryCast) == 0) return;
		// check if could cast directly to enclosing cast type, without intermediate type cast
		AsExpression alternateCast = new AsExpression(null, enclosingCast.type);
		alternateCast.resolvedType = enclosingCast.resolvedType;
		if (!alternateCast.checkCastTypesCompatibility(scope, enclosingCast.resolvedType, nestedCast.expression.resolvedType, null /* no expr to avoid side-effects*/)) return;
		scope.problemReporter().unnecessaryAs(nestedCast);
	}
	/**
			...
			if (expressionType != null) {
				...
				if (isLegal) {
					...
					if ((this.bits & ASTNode.UnsafeCast) != 0) { // unsafe cast
						...
						scope.problemReporter().unsafeNullnessAs(this, scope);
					} else {
						...
						if ((this.bits & (ASTNode.UnnecessaryCast|ASTNode.DisableUnnecessaryCastCheck)) == ASTNode.UnnecessaryCast) { // unnecessary cast
							if (!isIndirectlyUsed()) // used for generic type inference or boxing ?
								scope.problemReporter().unnecessaryAs(this);
						}
					}
				} else { // illegal cast
					if ((castType.tagBits & TagBits.HasMissingType) == 0) { // no complaint if secondary error
						scope.problemReporter().typeAsError(this, castType, expressionType);
					}
