				...
				case 20: //cym 2015-01-15
					if (CharOperation.equals(typeName, ConstantPool.JAVA_LANG_OVERLOAD)) {
						...
						for (int i = 0; i < numberOfPairs; i++) {
							...
							char[] membername = utf8At(utf8Offset + 3, u2At(utf8Offset + 1));
							currentOffset += 2;
							u1At(currentOffset);  // tag
							currentOffset++;
							int constValueOffset = -1;
							constValueOffset = this.constantPoolOffsets[u2At(currentOffset)] - this.structOffset;
							Object value = StringConstant.fromValue(String.valueOf(utf8At(constValueOffset + 3, u2At(constValueOffset + 1))));
							currentOffset += 2;
							//
							this.pairs[i] = new ElementValuePairInfo(membername, value);
						}
