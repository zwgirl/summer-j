...
class TypeBinding implements ITypeBinding {
	private static final StringLiteral EXPRESSION = new org.summer.sdt.internal.compiler.ast.StringLiteral(0,0);
	...
	org.summer.sdt.internal.compiler.lookup.TypeBinding binding;
	...
	public TypeBinding(BindingResolver resolver, org.summer.sdt.internal.compiler.lookup.TypeBinding binding) {
		this.binding = binding;
		this.resolver = resolver;
		org.summer.sdt.internal.compiler.lookup.TypeBinding compilerPrototype = binding.prototype();
	...
	public IAnnotationBinding[] getAnnotations() {
		...
		org.summer.sdt.internal.compiler.lookup.ReferenceBinding refType = null;
		if (this.binding instanceof ParameterizedTypeBinding) {
			...
			refType = (org.summer.sdt.internal.compiler.lookup.ReferenceBinding) this.binding;
		}
		if (refType != null) {
	...
	private IAnnotationBinding[] resolveAnnotationBindings(org.summer.sdt.internal.compiler.lookup.AnnotationBinding[] internalAnnotations, boolean isTypeUse) {
	...
	public String getBinaryName() {
		if (this.binding.isCapture()) {
			...
			org.summer.sdt.internal.compiler.lookup.Binding declaring = typeVariableBinding.declaringElement;
			StringBuffer binaryName = new StringBuffer();
			switch(declaring.kind()) {
				case org.summer.sdt.internal.compiler.lookup.Binding.METHOD :
	...
	public synchronized IMethodBinding[] getDeclaredMethods() {
		...
		try {
			if (isClass() || isInterface() || isEnum()) {
				ReferenceBinding referenceBinding = (ReferenceBinding) this.binding;
				org.summer.sdt.internal.compiler.lookup.MethodBinding[] internalMethods = referenceBinding.availableMethods(); // be resilient
				int length = internalMethods.length;
				if (length != 0) {
					int convertedMethodCount = 0;
					IMethodBinding[] newMethods = new IMethodBinding[length];
					for (int i = 0; i < length; i++) {
						org.summer.sdt.internal.compiler.lookup.MethodBinding methodBinding = internalMethods[i];
	...
	private JavaElement getUnresolvedJavaElement(org.summer.sdt.internal.compiler.lookup.TypeBinding typeBinding ) {
	...
	public ITypeBinding[] getTypeArguments() {
		...
		if (this.binding.isParameterizedTypeWithActualArguments()) {
			ParameterizedTypeBinding parameterizedTypeBinding = (ParameterizedTypeBinding) this.binding;
			final org.summer.sdt.internal.compiler.lookup.TypeBinding[] arguments = parameterizedTypeBinding.arguments;
	...
	public ITypeBinding[] getTypeBounds() {
		...
		if (typeVariableBinding != null) {
			...
			org.summer.sdt.internal.compiler.lookup.TypeBinding firstClassOrArrayBound = typeVariableBinding.firstBound;
			int boundsLength = 0;
			if (firstClassOrArrayBound == null) {
				...
				if (org.summer.sdt.internal.compiler.lookup.TypeBinding.equalsEquals(firstClassOrArrayBound, varSuperclass)) {
					boundsLength++;
	...
	public boolean isEqualTo(IBinding other) {
		...
		org.summer.sdt.internal.compiler.lookup.TypeBinding otherBinding = ((TypeBinding) other).binding;
		if (org.summer.sdt.internal.compiler.lookup.TypeBinding.equalsEquals(otherBinding.unannotated(), this.binding.unannotated())) {
			return true;
		}
		// check return type
	...
	public boolean isFromSource() {
		if (isClass() || isInterface() || isEnum()) {
			...
			if (referenceBinding.isRawType()) {
				...
				org.summer.sdt.internal.compiler.lookup.TypeBinding erasure = parameterizedTypeBinding.erasure();
			...
			if (declaringElement instanceof MethodBinding) {
				...
				final org.summer.sdt.internal.compiler.lookup.TypeBinding typeBinding = (org.summer.sdt.internal.compiler.lookup.TypeBinding) declaringElement;
				if (typeBinding instanceof ReferenceBinding) {
					...
					final org.summer.sdt.internal.compiler.lookup.TypeBinding leafComponentType = arrayBinding.leafComponentType;
	...
	public boolean isNullType() {
		return this.binding == org.summer.sdt.internal.compiler.lookup.TypeBinding.NULL;
	}
