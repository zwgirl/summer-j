...
import org.summer.sdt.internal.codeassist.ISelectionRequestor;
...
public void acceptLocalVariable(LocalVariableBinding binding, org.summer.sdt.internal.compiler.env.ICompilationUnit unit) {
	...
	if(binding.declaringScope == null){
		...
		parent = this.handleFactory.createElement(binding.windowScope, local.sourceStart, (ICompilationUnit) unit, existingElements, knownScopes);
	} else 	if (binding.declaringScope.isLambdaSubscope() && unit instanceof ICompilationUnit) {
		HashSet existingElements = new HashSet();
		HashMap knownScopes = new HashMap();
...
protected IJavaElement findLocalElement(int pos, MethodScope scope) {
	if (scope != null && scope.isLambdaScope()) {
		...
		if (expression != null && expression.resolvedType != null && expression.resolvedType.isValidBinding()) {
			org.summer.sdt.internal.core.LambdaExpression lambdaElement = LambdaFactory.createLambdaExpression((JavaElement) parent, expression);
