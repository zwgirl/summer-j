...
import org.summer.sdt.core.util.IClassFileAttribute;
import org.summer.sdt.core.util.IClassFileReader;
import org.summer.sdt.core.util.ICodeAttribute;
import org.summer.sdt.core.util.IFieldInfo;
import org.summer.sdt.core.util.IMethodInfo;
...
import org.summer.sdt.internal.compiler.env.IDependent;
...
public class Util {
	...
	private static IFile findFirstClassFile(IFolder folder) {
		try {
			...
			for (int i = 0, max = members.length; i < max; i++) {
				...
				} else if (org.summer.sdt.internal.compiler.util.Util.isClassFileName(member.getName())) {
					return (IFile) member;
				}
			}
	...
	public static String getLineSeparator(String text, IJavaProject project) {
		...
		return org.summer.sdt.internal.compiler.util.Util.LINE_SEPARATOR;
	}
				...
				case ARGUMENTS_DELIMITER :
					...
					if (next == ARGUMENTS_DELIMITER) {
						...
						if (EMPTY_ARGUMENT.equals(currentArgumentContents)) {
							currentArgumentContents = org.summer.sdt.internal.compiler.util.Util.EMPTY_STRING;
						}
	...
	public static byte[] getResourceContentsAsByteArray(IFile file) throws JavaModelException {
		...
		try {
			return org.summer.sdt.internal.compiler.util.Util.getInputStreamAsByteArray(stream, -1);
		} catch (IOException e) {
	...
	public static char[] getResourceContentsAsCharArray(IFile file, String encoding) throws JavaModelException {
		...
		try {
			return org.summer.sdt.internal.compiler.util.Util.getInputStreamAsCharArray(stream, (int) length, encoding);
		} catch (IOException e) {
			throw new JavaModelException(e, IJavaModelStatusConstants.IO_EXCEPTION);
		} finally {
			try {
	...
	public static JavaElement getUnresolvedJavaElement(MethodBinding methodBinding, WorkingCopyOwner workingCopyOwner, BindingsToNodesMap bindingsToNodes) {
		...
		org.summer.sdt.internal.compiler.ast.ASTNode node = bindingsToNodes == null ? null : bindingsToNodes.get(methodBinding);
	...
	public static JavaElement getUnresolvedJavaElement(TypeBinding typeBinding, WorkingCopyOwner workingCopyOwner, BindingsToNodesMap bindingsToNodes) {
		...
		if (referenceBinding.isLocalType() || referenceBinding.isAnonymousType()) {
			...
			if (org.summer.sdt.internal.compiler.util.Util.isClassFileName(fileName)) {
				int jarSeparator = CharOperation.indexOf(IDependent.JAR_FILE_ENTRY_SEPARATOR, fileName);
				int pkgEnd = CharOperation.lastIndexOf('/', fileName); // pkgEnd is exclusive
				if (pkgEnd == -1)
					pkgEnd = CharOperation.lastIndexOf(File.separatorChar, fileName);
			...
			try {
				int sourceStart = ((org.summer.sdt.internal.compiler.lookup.LocalTypeBinding) referenceBinding).sourceStart;
			...
			org.summer.sdt.internal.compiler.lookup.Binding declaringElement = ((org.summer.sdt.internal.compiler.lookup.TypeVariableBinding) referenceBinding).declaringElement;
	...
	public final static boolean isExcluded(IPath resourcePath, char[][] inclusionPatterns, char[][] exclusionPatterns, boolean isFolderPath) {
		...
		return org.summer.sdt.internal.compiler.util.Util.isExcluded(resourcePath.toString().toCharArray(), inclusionPatterns, exclusionPatterns, isFolderPath);
	}
	...
	public static int scanTypeSignature(char[] string, int start) {
		...
		return org.summer.sdt.internal.compiler.util.Util.scanTypeSignature(string, start);
	}
	/**
	...
	static void appendTypeSignature(char[] string, int start, StringBuffer buffer, boolean compact) {
		...
		switch (c) {
			...
			case Signature.C_TYPE_VARIABLE :
				int e = org.summer.sdt.internal.compiler.util.Util.scanTypeVariableSignature(string, start);
	...
	public static String typeSignature(TypeReference type) {
		...
		if ((type.bits & org.summer.sdt.internal.compiler.ast.ASTNode.IsUnionType) != 0) {
	...
	public static IAnnotation getAnnotation(JavaElement parent, IBinaryAnnotation binaryAnnotation, String memberValuePairName) {
		char[] typeName = org.summer.sdt.core.Signature.toCharArray(CharOperation.replaceOnCopy(binaryAnnotation.getTypeName(), '/', '.'));
