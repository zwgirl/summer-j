...
import org.summer.sdt.core.util.*;
...
public class Disassembler extends ClassFileBytesDisassembler {
	...
	static String decodeStringValue(char[] chars) {
		...
		for (int i = 0, max = chars.length; i < max; i++) {
			char c = chars[i];
			org.summer.sdt.internal.compiler.util.Util.appendEscapedChar(buffer, c, true);
		}
		return buffer.toString();
	}

	private static void escapeChar(StringBuffer buffer, char c) {
		org.summer.sdt.internal.compiler.util.Util.appendEscapedChar(buffer, c, false);
	}
	...
	/**
	 * @see org.summer.sdt.core.util.ClassFileBytesDisassembler#disassemble(byte[], java.lang.String)
	 */
	...
	/**
	 * @see org.summer.sdt.core.util.ClassFileBytesDisassembler#disassemble(byte[], java.lang.String, int)
	 */
	...
	/**
	 * @see #disassemble(org.summer.sdt.core.util.IClassFileReader, java.lang.String, int)
	 */
	...
	public String disassemble(IClassFileReader classFileReader, String lineSeparator, int mode) {
		if (classFileReader == null) return org.summer.sdt.internal.compiler.util.Util.EMPTY_STRING;
		char[] className = classFileReader.getClassName();
		if (className == null) {
			// incomplete initialization. We cannot go further.
			return org.summer.sdt.internal.compiler.util.Util.EMPTY_STRING;
		}
		...
		if (checkMode(mode, SYSTEM | DETAILED)) {
			...
			buffer.append(
				...
				new String[] {
					...
					+ (isDeprecated(classFileReader) ? ", deprecated" : org.summer.sdt.internal.compiler.util.Util.EMPTY_STRING)//$NON-NLS-1$
				}));
	...
	/**
	 * @see org.summer.sdt.core.util.ClassFileBytesDisassembler#getDescription()
	 */
