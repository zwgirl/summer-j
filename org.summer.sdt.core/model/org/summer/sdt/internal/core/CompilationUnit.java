...
//		parser.setStatementsRecovery((reconcileFlags & ICompilationUnit.ENABLE_STATEMENTS_RECOVERY) != 0);
		...
		try {
			...
			if (createAST) {
				...
				org.summer.sdt.core.dom.CompilationUnit cu = AST.convertCompilationUnit(astLevel, compilationUnitDeclaration, options, computeProblems, source, reconcileFlags, pm);
	...
	public void codeComplete(int offset, ICompletionRequestor requestor, WorkingCopyOwner workingCopyOwner) throws JavaModelException {
		...
		codeComplete(offset, new org.summer.sdt.internal.codeassist.CompletionRequestorWrapper(requestor), workingCopyOwner);
	}
	/**
	...
	public org.summer.sdt.core.dom.CompilationUnit makeConsistent(int astLevel, boolean resolveBindings, int reconcileFlags, HashMap problems, IProgressMonitor monitor) throws JavaModelException {
		...
		try {
			...
			if (astLevel != NO_AST || problems != null) {
				...
				org.summer.sdt.core.dom.CompilationUnit result = info.ast;
				info.ast = null;
				return result;
			} else {
	 ...
	 * @since 3.0
	 */
	public org.summer.sdt.core.dom.CompilationUnit reconcile(
			int astLevel,
