...
import org.summer.sdt.internal.compiler.env.IGenericType;
...
public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding, AccessRestriction accessRestriction) {
	...
	if (unit != null) {
		try {
			...
			org.summer.sdt.core.ICompilationUnit cu = ((SourceTypeElementInfo)sourceType).getHandle().getCompilationUnit();
 ...
 */
private void rememberAllTypes(CompilationUnitDeclaration parsedUnit, org.summer.sdt.core.ICompilationUnit cu, boolean includeLocalTypes) {
...
public void resolve(IGenericType suppliedType) {
	try {
		if (suppliedType.isBinaryType()) {
			...
			org.summer.sdt.core.ICompilationUnit cu = ((SourceTypeElementInfo)suppliedType).getHandle().getCompilationUnit();
			if (cu != null) {
...
public void resolve(Openable[] openables, HashSet localTypes, IProgressMonitor monitor) {
	try {
		...
		org.summer.sdt.core.ICompilationUnit[] cus = new org.summer.sdt.core.ICompilationUnit[openablesLength];
		int unitsIndex = 0;
		...
		for (int i = 0; i < openablesLength; i++) {
			...
			if (openable instanceof org.summer.sdt.core.ICompilationUnit) {
				org.summer.sdt.core.ICompilationUnit cu = (org.summer.sdt.core.ICompilationUnit)openable;
