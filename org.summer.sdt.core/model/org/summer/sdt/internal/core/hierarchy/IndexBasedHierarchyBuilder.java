 ...
 *******************************************************************************/
package org.summer.sdt.internal.core.hierarchy;
...
import org.summer.sdt.core.search.*;
...
import org.summer.sdt.internal.core.search.IndexQueryRequestor;
import org.summer.sdt.internal.core.search.JavaSearchParticipant;
import org.summer.sdt.internal.core.search.SubTypeSearchJob;
...
import org.summer.sdt.internal.core.search.matching.MatchLocator;
import org.summer.sdt.internal.core.search.matching.SuperTypeReferencePattern;
...
public class IndexBasedHierarchyBuilder extends HierarchyBuilder implements SuffixConstants {
	...
	private void buildForProject(JavaProject project, ArrayList potentialSubtypes, org.summer.sdt.core.ICompilationUnit[] workingCopies, HashSet localTypes, IProgressMonitor monitor) throws JavaModelException {
		// resolve
		...
		if (openablesLength > 0) {
			...
			org.summer.sdt.core.ICompilationUnit[] unitsToLookInside = null;
			if (inProjectOfFocusType) {
				org.summer.sdt.core.ICompilationUnit unitToLookInside = focusType.getCompilationUnit();
				if (unitToLookInside != null) {
					int wcLength = workingCopies == null ? 0 : workingCopies.length;
					if (wcLength == 0) {
						unitsToLookInside = new org.summer.sdt.core.ICompilationUnit[] {unitToLookInside};
					} else {
						unitsToLookInside = new org.summer.sdt.core.ICompilationUnit[wcLength+1];
	...
	private void buildFromPotentialSubtypes(String[] allPotentialSubTypes, HashSet localTypes, IProgressMonitor monitor) {
		...
		int wcLength;
		org.summer.sdt.core.ICompilationUnit[] workingCopies = this.hierarchy.workingCopies;
		if (workingCopies != null && (wcLength = workingCopies.length) > 0) {
			String[] newPaths = new String[wcLength];
			for (int i = 0; i < wcLength; i++) {
				org.summer.sdt.core.ICompilationUnit workingCopy = workingCopies[i];
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				try {
					...
					Openable handle;
					org.summer.sdt.core.ICompilationUnit workingCopy = (org.summer.sdt.core.ICompilationUnit)wcPaths.get(resourcePath);
					if (workingCopy != null) {
