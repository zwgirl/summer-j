...
import org.summer.sdt.internal.compiler.ISourceElementRequestor;
...
import org.summer.sdt.internal.core.index.*;
import org.summer.sdt.internal.core.search.BasicSearchEngine;
import org.summer.sdt.internal.core.search.PatternSearchJob;
import org.summer.sdt.internal.core.search.processing.IJob;
...
private char[][] readJavaLikeNamesFile() {
	try {
		...
		if (!javaLikeNamesFile.exists())
			return null;
		char[] javaLikeNames = org.summer.sdt.internal.compiler.util.Util.getFileCharContent(javaLikeNamesFile, null);
...
private void readIndexMap() {
	try {
		char[] indexMaps = org.summer.sdt.internal.compiler.util.Util.getFileCharContent(this.indexNamesMapFile, null);
		char[][] names = CharOperation.splitOn('\n', indexMaps);
		if (names.length >= 3) {
...
private char[][] readIndexState(String dirOSString) {
	try {
		char[] savedIndexNames = org.summer.sdt.internal.compiler.util.Util.getFileCharContent(this.savedIndexNamesFile, null);
...
private void readParticipantsIndexNamesFile() {
	...
	try {
		char[] participantIndexNames = org.summer.sdt.internal.compiler.util.Util.getFileCharContent(this.participantIndexNamesFile, null);
