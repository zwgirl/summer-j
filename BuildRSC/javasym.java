interface javasym
{
    public final static int
      TokenNameIdentifier = 13,
      TokenNameabstract = 50,
      TokenNameboolean = 98,
      TokenNamebreak = 88,
      TokenNamebyte = 99,
      TokenNamecase = 100,
      TokenNamecatch = 111,
      TokenNamechar = 101,
      TokenNameclass = 68,
      TokenNamecontinue = 89,
      TokenNameconst = 126,
      TokenNamedebugger = 127,
      TokenNamedefault = 90,
      TokenNamedo = 91,
      TokenNamedouble = 102,
      TokenNameelse = 112,
      TokenNameenum = 71,
      TokenNameextends = 97,
      TokenNamefalse = 43,
      TokenNamefinal = 51,
      TokenNamefinally = 113,
      TokenNamefloat = 103,
      TokenNamefor = 75,
      TokenNamefunction = 114,
      TokenNamegoto = 128,
      TokenNameif = 73,
      TokenNameimplements = 123,
      TokenNameimport = 104,
      TokenNamein = 117,
      TokenNameinstanceof = 16,
      TokenNameint = 105,
      TokenNameinterface = 70,
      TokenNamelet = 129,
      TokenNamelong = 106,
      TokenNamenative = 52,
      TokenNamenew = 37,
      TokenNamenull = 53,
      TokenNamepackage = 96,
      TokenNameprivate = 54,
      TokenNameprotected = 55,
      TokenNamepublic = 56,
      TokenNamereturn = 92,
      TokenNameshort = 107,
      TokenNamestatic = 41,
      TokenNamestrictfp = 57,
      TokenNamesuper = 38,
      TokenNameswitch = 74,
      TokenNamesynchronized = 58,
      TokenNamethis = 36,
      TokenNamethrow = 93,
      TokenNamethrows = 115,
      TokenNametransient = 59,
      TokenNametrue = 44,
      TokenNametry = 94,
      TokenNametypeof = 130,
      TokenNamevar = 109,
      TokenNamevoid = 108,
      TokenNamevolatile = 60,
      TokenNamewhile = 76,
      TokenNamePCDATA = 118,
      TokenNameIntegerLiteral = 42,
      TokenNameLongLiteral = 45,
      TokenNameFloatingPointLiteral = 46,
      TokenNameDoubleLiteral = 47,
      TokenNameCharacterLiteral = 48,
      TokenNameStringLiteral = 49,
      TokenNameCODE_DATA = 119,
      TokenNamePLUS_PLUS = 2,
      TokenNameMINUS_MINUS = 3,
      TokenNameEQUAL_EQUAL = 22,
      TokenNameLESS_EQUAL = 17,
      TokenNameGREATER_EQUAL = 18,
      TokenNameNOT_EQUAL = 23,
      TokenNameLEFT_SHIFT = 20,
      TokenNameRIGHT_SHIFT = 14,
      TokenNameUNSIGNED_RIGHT_SHIFT = 15,
      TokenNamePLUS_EQUAL = 77,
      TokenNameMINUS_EQUAL = 78,
      TokenNameMULTIPLY_EQUAL = 79,
      TokenNameDIVIDE_EQUAL = 80,
      TokenNameAND_EQUAL = 81,
      TokenNameOR_EQUAL = 82,
      TokenNameXOR_EQUAL = 83,
      TokenNameREMAINDER_EQUAL = 84,
      TokenNameLEFT_SHIFT_EQUAL = 85,
      TokenNameRIGHT_SHIFT_EQUAL = 86,
      TokenNameUNSIGNED_RIGHT_SHIFT_EQUAL = 87,
      TokenNameOR_OR = 31,
      TokenNameAND_AND = 30,
      TokenNamePLUS = 4,
      TokenNameMINUS = 1,
      TokenNameNOT = 64,
      TokenNameREMAINDER = 8,
      TokenNameXOR = 27,
      TokenNameAND = 21,
      TokenNameMULTIPLY = 7,
      TokenNameOR = 28,
      TokenNameTWIDDLE = 65,
      TokenNameDIVIDE = 9,
      TokenNameGREATER = 11,
      TokenNameLESS = 6,
      TokenNameLPAREN = 19,
      TokenNameRPAREN = 26,
      TokenNameLBRACE = 39,
      TokenNameRBRACE = 34,
      TokenNameLBRACKET = 10,
      TokenNameRBRACKET = 67,
      TokenNameSEMICOLON = 24,
      TokenNameQUESTION = 29,
      TokenNameCOLON = 62,
      TokenNameCOMMA = 35,
      TokenNameDOT = 5,
      TokenNameEQUAL = 69,
      TokenNameAT = 40,
      TokenNameELLIPSIS = 124,
      TokenNameARROW = 110,
      TokenNameCOLON_COLON = 12,
      TokenNameBeginLambda = 61,
      TokenNameBeginIntersectionCast = 66,
      TokenNameBeginTypeArguments = 95,
      TokenNameElidedSemicolonAndRightBrace = 72,
      TokenNameCLOSE_TAG = 116,
      TokenNameCLOSE_ELEMENT = 120,
      TokenNameAT308 = 25,
      TokenNameEQUAL_EQUAL_EQUAL = 32,
      TokenNameNOT_EQUAL_EQUAL = 33,
      TokenNameSCRIPT_START = 125,
      TokenNameSCRIPT_END = 121,
      TokenNameXmlComment = 122,
      TokenName$eof = 63,
      TokenName$error = 131;
}
